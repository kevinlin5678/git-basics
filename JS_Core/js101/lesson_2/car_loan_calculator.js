...
6. Try to print the payment amount as a dollar and cents amount,
   e.g., $123.45 or $371.00

let m = p * (j / (1 - Math.pow((1 + j), (-n))));
^ formula for monthly interest payment calculator
...
'welcome to interest payment calculator 1.0, please follow the instructions
and input your date to figure out your monthly payment!'

to figure out for 'p': 'what is the total of your loan amount?'

to figure out for 'APR': 'what is your Annual Percentage Rate (APR)?'

to figure out loan duration in 'loanDuration':
'what is the duration of your loan in months?'
...
function invalidNumber(num) {
  return num.trimStart() === '' || Number.isNaN(Number(num)) || Number(num) < 0 || !Number.isFinite(Number(num));
}
...
if (Number(APR) === 0) {
  monthlyPayment = totalLoanAmount / loanDuration;
} else {
  monthlyPayment =
  totalLoanAmount *
  (monthlyInterestRate /
