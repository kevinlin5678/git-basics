...
Each UUID consists of 32 hexadecimal characters (the digits 0-9 and the letters a-f) represented as a string. The value is typically broken into 5 sections in an 8-4-4-4-12 pattern, 
e.g., 'f65c57f6-a6aa-17a8-faa1-a67f2dc9fa91'.

Write a function that takes no arguments and returns a string that contains a UUID.
*/

function generateIUUD() {
  let characters = ['a', 'b', 'c', 'd', 'e', 'f', 1, 2, 3, 4, 5, 6, 7, 8, 9, 0]
  let sections = [8, 4, 4, 4, 12];
  let iuud = ''
  sections.forEach((section, index) => {
    for (let i = 0; i < section; i++) {
      iuud += characters[Math.floor(Math.random() * characters.length)];
    }
